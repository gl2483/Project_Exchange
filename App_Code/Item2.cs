using System;
using System.Collections.Generic;
using System.Linq;
using System.Web;
using System.Data;

/// <summary>
/// Summary description for Item2
/// </summary>
public class Item2
{
    public int ItemId { get; set; }
    public string ItemImage { get; set; }
    public string ItemName { get; set; }
    public string ItemDesc { get; set; }
    public string Category { get; set; }
    public DateTime CreatedOn { get; set; }
    public string UserName { get; set; }
    public bool IsActive { get; set; }
    public int UserId { get; set; }
    public bool showLike { get; set; }
    public bool showDislike { get; set; }

    //private const string notAvail = "data:image/jpg;base64,";
    private const string notAvail = "~/NotAvailable.jpg";

    public Item2(DataRow row, int pCurrentUserId)
	{
        ItemId = Convert.ToInt32(row["ItemId"]);
        ItemImage = row["Image"] == DBNull.Value ? notAvail : "data:" + row["ContentType"].ToString() + ";base64," + Convert.ToBase64String((byte[])row["Image"]);
        ItemName = row["ItemName"].ToString();
        ItemDesc = row["ItemDesc"].ToString();
        Category = row["Category"].ToString();
        CreatedOn = Convert.ToDateTime(row["CreatedOn"]);
        UserName = row["UserName"].ToString();
        IsActive = Convert.ToBoolean(row["IsActive"]);
        UserId = Convert.ToInt32(row["UserId"]);
        showLike = (Convert.ToInt32(row["UserId"]) != pCurrentUserId && pCurrentUserId != 0) && (row["Preference"] == DBNull.Value ? true : Convert.ToBoolean(row["Preference"]));
        showDislike = (Convert.ToInt32(row["UserId"]) != pCurrentUserId && pCurrentUserId != 0) && (row["Preference"] == DBNull.Value ? true : !Convert.ToBoolean(row["Preference"]));
		//
		// TODO: Add constructor logic here
		//
	}
}